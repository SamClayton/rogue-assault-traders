...
adminlog(LOG_RAW,"<font color=yellow><B>Combat Start:</B></font><BR>Attacker " . get_player($playerinfo['player_id']) . " $l_cmb_beams:<B> ".$attacker_beam_energy.
"</B>, $l_cmb_fighters: <B>".$attackerfighters."</B>, $l_cmb_shields: <B>$attacker_shield_energy</B>, $l_cmb_torps: <B>$attackertorps</B>. $l_cmb_armor: <B>$attackerarmor</B>
<br>Shield Tech: $shipinfo[shields] , Armor Tech: $shipinfo[armour] , Torp Tech: $shipinfo[torp_launchers] , Beam Tech: $shipinfo[beams] , Computer Tech: $shipinfo[computer]<br>
<br>Defender ".$targetname . " $l_cmb_beams=<B>".$target_beam_energy."</B>, $l_cmb_fighters: <B>".$targetfighters."</B>, $l_cmb_shields: <B>$target_shield_energy</B>, $l_cmb_torps: <B>$targettorps</B>. $l_cmb_armor: <B>$targetarmour</B>
<br>Shield Tech: $targetshipshields , Armor Tech: $targetshiparmour , Torp Tech: $targetshiptorp_launchers , Beam Tech: $targetshipbeams , Computer Tech: $targetshipcomputer , Base Factor: $base_factor");
adminlog(LOG_RAW,"<font color=yellow><B>Combat Class Values:</B></font><BR>Attacker " . get_player($playerinfo['player_id']) . " fighter_damage_shields:<B> ".$fighter_damage_shields.
...
if($attacker_beam_energy != 0)
{
	...
	//
	$target_armor = calc_failure($targetarmour, $targetshiparmour, $shipinfo['beams']);

	$target_armor_hit_pts = $targetarmour * $ship_armor_hit_pts;

	$attack_energy_left = $attack_fire_damage[1];
	...
	if($attack_fire_damage[0] > $target_shield_hit_pts)
	{
		...
		$attack_energy_left = floor($attack_fire_damage[0] / $beam_damage_shields);
		$attack_fire2_damage = calc_damage($attack_energy_left, $beam_damage_all, $attackerlowpercent, $shipinfo['beams'], $targetshiparmour);
		$attack_energy_left += $attack_fire2_damage[1];
		...
		if($attack_fire2_damage[0] > $target_armor_hit_pts)
		{
			...
			$attack_energy_left += floor($attack_fire2_damage[0] / $beam_damage_all);
			$target_armor_left = 0;
		}
		else
		{
	...
	else
	{
		...
		$target_armor_left = $targetarmour;
	}
}
else
{
	...
	$target_armor_left = $targetarmour;
	$attack_energy_left = 0;
}

if($attack_beamtofighter_dmg != 0)
{
	$attack_fighter_damage = calc_damage($attack_beamtofighter_dmg, $beam_damage_all, $attackerlowpercent, $shipinfo['beams'], $targetshipcomputer);
	...
	$attack_energy_left += $attack_fighter_damage[1];
	...
	if($attack_fighter_damage[0] > $target_fighter_hit_pts)
	{
		...
		$attack_energy_left += floor($attack_fighter_damage[0] / $beam_damage_all);
...
if($attack_beamtotorp_dmg != 0)
{
	...
	$attack_energy_left += $attack_torp_damage[1];
	...
	if($attack_torp_damage[0] > $target_torp_hit_pts)
	{
		...
		$attack_energy_left += floor($attack_torp_damage[0] / $beam_damage_all);
		$target_torps_left = 0;
	}
	else
	{
...
if($target_fighters_left != 0)
{
	$target_fighter_damage = calc_damage($target_fighters_left, $fighter_damage_all, $targetlowpercent, $targetshipcomputer, $shipinfo['computer']);
...
if($attacker_torps_left2 != 0)
{
	...
	//
	$target_armor = calc_failure($target_armor_left, $targetshiparmour, $shipinfo['torp_launchers']);
	...
	if($attack_torp_damage[0] > $target_shield_hit_pts)
	{
		...
		$attack_torp2_damage = calc_damage($attacker_torps_left2, $torp_damage_all, $attackerlowpercent, $shipinfo['torp_launchers'], $targetshiparmour);
...
if($target_torps_left2 != 0)
{
	...
	if($target_torp_damage[0] > $attack_shield_hit_pts)
	{
		...
		$target_torp2_damage = calc_damage($target_torps_left2, $torp_damage_all, $targetlowpercent, $targetshiptorp_launchers, $shipinfo['armour']);
...
adminlog(LOG_RAW,"<font color=yellow><B>Combat End:</B></font><BR>Attacker " . get_player($playerinfo['player_id']) . " $l_cmb_beams=<B>".$attack_energy_left.
