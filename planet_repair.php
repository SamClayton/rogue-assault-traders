...
// Computer
		...
		if($planetinfo['computer'] != $planetinfo['computer_normal']){
			...
			{
				$computer_upgrade_cost = phpChangeDelta($computer_upgrade, $planetinfo['computer']);
			}
		}
		else
		{
			$computer_upgrade = $planetinfo['computer'];
		}
...
// Sensors
		...
		if($planetinfo['sensors'] != $planetinfo['sensors_normal']){
			...
			{
				$sensors_upgrade_cost = phpChangeDelta($sensors_upgrade, $planetinfo['sensors']);
			}
		}
		else
		{
			$sensors_upgrade = $planetinfo['sensors'];
		}
...
// Beams
		...
		if($planetinfo['beams'] != $planetinfo['beams_normal']){
			...
			{
				$beams_upgrade_cost = phpChangeDelta($beams_upgrade, $planetinfo['beams']);
			}
		}
		else
		{
			$beams_upgrade = $planetinfo['beams'];
		}
...
// Cloak
		...
		if($planetinfo['cloak'] != $planetinfo['cloak_normal']){
			...
			{
				$cloak_upgrade_cost = phpChangeDelta($cloak_upgrade, $planetinfo['cloak']);
			}
		}
		else
		{
			$cloak_upgradegrade = $planetinfo['cloak'];
		}
...
// Torp_launchers
		...
		if($planetinfo['torp_launchers'] != $planetinfo['torp_launchers_normal']){
			...
			{
				$torp_launchers_upgrade_cost = phpChangeDelta($torp_launchers_upgrade, $planetinfo['torp_launchers']);
			}
		}
		else
		{
			$torp_launchers_upgrade = $planetinfo['torp_launchers'];
		}
...
// Shields
		...
		if($planetinfo['shields'] != $planetinfo['shields_normal']){
			...
			{
				$shields_upgrade_cost = phpChangeDelta($shields_upgrade, $planetinfo['shields']);
			}
		}
		else
		{
			$shields_upgrade = $planetinfo['shields'];
		}
...
// Jammer
		...
		{
			$jammer_upgrade = $planetinfo['jammer'];
		}

		$total_cost = floor(($computer_upgrade_cost +
		$sensors_upgrade_cost + $beams_upgrade_cost + $cloak_upgrade_cost +
		$torp_launchers_upgrade_cost + $shields_upgrade_cost+ $jammer_upgrade_cost) * ($repair_modifier / 100));
...
// Computer
			...
			{
				$max_credits += phpChangeDelta($planetinfo['computer'], 0);
			}
...
// Sensors
			...
			{
				$max_credits += phpChangeDelta($planetinfo['sensors'], 0);
			}
...
// Beam Weapons
			...
			{
				$max_credits += phpChangeDelta($planetinfo['beams'], 0);
			}
...
// Torpedo Launchers
			...
			{
				$max_credits += phpChangeDelta($planetinfo['torp_launchers'], 0);
			}
...
// Shields
			...
			{
				$max_credits += phpChangeDelta($planetinfo['shields'], 0);
			}

// Jammer
			if ($jammer_upgrade > $planetinfo['jammer'])
			{
				$query = $query . ", jammer=$jammer_upgrade";
				$upgradejammer = 1;
				$max_credits += phpChangeDelta($jammer_upgrade, 0);
   			}
			else
			{
				$max_credits += phpChangeDelta($planetinfo['jammer'], 0);
			}
...
// Cloak
			...
			{
				$max_credits += phpChangeDelta($planetinfo['cloak'], 0);
			}
