...
function cheater_found(){
	global $bet, $cash, $dealerout, $playersplitout, $playerout, $playerinfo, $shipinfo, $db, $dbtables;
	$bet = 0;
	$cash /= 2;
	...
	$res = $db->Execute("SELECT character_name FROM $dbtables[players] WHERE player_id = 1");
	$attackername = $res->fields['character_name'];
	$mystuff = $attackername . "|" . ($cash / 2) . "|" . $playerinfo['character_name'];
	insert_news($mystuff, 1, "bounty");

	$debug_query = $db->Execute("INSERT INTO $dbtables[bounty] (bounty_on,placed_by,amount) values ($playerinfo[player_id], 1," . ($cash / 2) . ")");
	db_op_result($debug_query,__LINE__,__FILE__);
	$res = $db->Execute("INSERT INTO $dbtables[shoutbox] (player_id,player_name,sb_date,sb_text,sb_alli) VALUES (1,'$attackername'," . time() . ",'$playerinfo[character_name] CHEATED the Casino in sector $shipinfo[sector_id]',0) ");
}
...
function card_value($cards){
	...
	for ($i=1;$i <=$numitems;$i++){
		$cardout=substr($newdeck[$cards[$i]],0,1);
...
function show_player_hand($playerout){
	...
	for ($i=1;$i <=$numitems;$i++){
		...
		$playerhandout.="<img src=\"templates/$templatename/images/casino/".$cardout.".gif\"><img src=\"templates/$templatename/images/casino/spacer.gif\" width=\"2\">";
	}
...
function show_dealer_hand($playerout,$flag){
	...
	for ($i=1;$i <=$numitems;$i++){
		if ($flag==0){
			if ($i >1){
				$cardout=($newdeck[$playerout[$i]]);
				$dealerhandout.="<img src=\"templates/$templatename/images/casino/".$cardout.".gif\"><img src=\"images/spacer.gif\" width=\"2\">";
			}else{
				$dealerhandout.="<img src=\"templates/$templatename/images/casino/card_down.gif\"><img src=\"images/spacer.gif\" width=\"2\">";
			}
		}else{
			$cardout=($newdeck[$playerout[$i]]);
			$dealerhandout.="<img src=\"templates/$templatename/images/casino/".$cardout.".gif\"><img src=\"images/spacer.gif\" width=\"2\">";		
...
switch ($action) {
	...
	case "Deal":
		...
		if ($bet_amt > $cash || $bet_amt > $max_casino_bet){
			...
			$dealerout=show_dealer_hand($dealer,0);
			$playerout=show_player_hand($player);
			$cardout=substr($newdeck[$dealer[2]],0,1);
			if ($cardout=="A"){
				$status="Insurance";
			}else{
				$status="Deal";
...
$debug_query = $db->Execute("UPDATE $dbtables[players] SET credits=$cash $deduct_turn WHERE player_id=$playerinfo[player_id]");
db_op_result($debug_query,__LINE__,__FILE__);

if ($playercards==2){
	$card1=substr($newdeck[$player[1]],0,1);
	$card2=substr($newdeck[$player[2]],0,1);
...
$smarty->assign("l_firsthand", $l_firsthand);	
$smarty->assign("l_secondhand", $l_secondhand);		
$smarty->assign("l_bet", $l_bet);	
$smarty->assign("l_stand",$l_stand);	
$smarty->assign("l_hit",$l_hit);	
$smarty->assign("l_insurance",$l_insurance );
$smarty->assign("l_doubledown",$l_doubledown);
$smarty->assign("l_lobby",$l_lobby);	
$smarty->assign("l_split",$l_split); 	
$smarty->assign("l_credits",$l_credits); 
$smarty->assign("l_yourbet",$l_yourbet);	
$smarty->assign("l_dealerbust",$l_dealerbust);
$smarty->assign("l_dealerwins",$l_dealerwins);
$smarty->assign("l_nobet",$l_nobet);
$smarty->assign("l_dealerloses",$l_dealerloses);
$smarty->assign("l_dealerblackjack",$l_dealerblackjack);
$smarty->assign("l_push",$l_push);		
$smarty->assign("l_playerwins",$l_playerwins);
$smarty->assign("l_playerbust",$l_playerbust);
$smarty->assign("l_playerloses",$l_playerloses);
$smarty->assign("l_playerblackjack",$l_playerblackjack);
$smarty->assign("l_place_bet",$l_place_bet);	
$smarty->assign("error_msg", $l_planet_none);
$smarty->assign("gotomain", $l_global_mmenu);

if ($split_flag > 1){
	$smarty->assign("bet", NUMBER($bet*2));
}else{
	$smarty->assign("bet", NUMBER($bet));
}

$smarty->assign("availcash", $availcash);
$smarty->assign("old_bet", $_SESSION['old_bet']);
$smarty->assign("hand", $hand);
$smarty->assign("playercards", $playercards);	
$smarty->assign("split_flag", $split_flag);	
$smarty->assign("cash", NUMBER($cash));
$smarty->assign("dealerout", $dealerout);
$smarty->assign("playerout", $playerout);
$smarty->assign("playersplitout", $playersplitout);
$smarty->assign("status", $status);
$smarty->assign("gotomain", $l_global_mmenu);
$smarty->display($templatename."casino_blackjack.tpl");
include ("footer.php");
?>
