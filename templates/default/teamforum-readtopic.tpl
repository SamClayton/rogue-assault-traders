...
<tr>
	  <td width="100%">
		...
		{php}
			...
			if($forumadmin == 1){
			echo "		  	 | <A class=\"but\" HREF=\"team-forum.php?command=delete&post_id=$postid[$i]&topic_id=$topic_id\">$l_readm_del</A>";
			}
			if(($forumadmin == 1 or $isposter[$i]) and $topic_status != 0){
			echo "		  	 | <A class=\"but\" HREF=\"team-forum.php?command=edit&post_id=$postid[$i]&topic_id=$topic_id\">$l_forums_edit</A>";
			}
			if($topic_status != 0){
			echo "			 | <A class=\"but\" HREF=\"team-forum.php?command=postreply&topic_id=$topic_id\">$l_readm_repl</A>";
			}
			echo " |";
			echo "		  </td>";
			echo "		</tr>";
			...
			echo "		<tr>";
			echo "		  <td width=\"100%\" align=\"center\" valign=\"middle\">";
			echo "		  	 | <A class=\"but\" HREF=\"team-forum.php?command=lock&topic_id=$topic_id\">$l_forums_lock</A> |";
			echo "		  </td>";
			echo "		</tr>";
			echo "	  </table>";
			echo "	</div>";
