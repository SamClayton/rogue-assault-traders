...
<tr valign=top><td><font size=2 face="courier new" color=#ff0000>{$l_igb_shipaccount} :</td><td align=right><font size=2 face="courier new" color=#ff0000>{$playercredits} C</td></tr>
<tr valign=top><td><font size=2 face="courier new" color=#ff0000>{$l_igb_currentloan} :</td><td align=right><font size=2 face="courier new" color=#ff0000>{$accountloan} C</td></tr>

{if $accountloan != 0}
	<tr valign=top><td nowrap><font size=2 face="courier new" color=#ff0000>{$l_igb_loantimeleft} :</td>

	{if $isloanlate}
	  <td align=right><font size=2 face="courier new" color=#ff0000>{$l_igb_loanlate}</td>
	{else}
	  echo "<td align=right><font size=2 face="courier new" color=#ff0000>{$hours}h {$mins}m</td>
	{/if}
	</tr>
	<form action=igb.php?command=repay method=POST>
	<tr valign=top>
	<td><br><font size=2 face="courier new" color=#ff0000>{$l_igb_repayamount} :</td>
...
{else}
	<tr valign=top><td><font size=2 face="courier new" color=#ff0000>{$l_igb_maxloanpercent} :</td><td align=right><font size=2 face="courier new" color=#ff0000>{$maxloan} C</td></tr>

	{if $iscollateral}
		<form action=igb.php?command=borrow method=POST>
		<tr valign=top>
		<td><br><font size=2 face="courier new" color=#ff0000>{$l_igb_loanamount} :</td>
	...
	{else}
		<tr valign=top>
		<td colspan=2 align=center><br><font size=2 face="courier new" color=#ff0000>{$l_igb_nocollateral}</font></td>
	{/if}
{/if}
