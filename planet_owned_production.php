...
// 
// File: planet_owned_production.php
...
if ($planetinfo['owner'] == $playerinfo['player_id'] || ($planetinfo['team'] == $playerinfo['team'] && $playerinfo['team'] > 0 && $planetinfo['owner'] > 0))
{
		...
		if ($porganics < 0.0 || $pore < 0.0 || $pgoods < 0.0 || $penergy < 0.0 || $pfighters < 0.0 || $ptorp < 0.0 || $presearch < 0.0 || $pbuild < 0.0)
		{
			$result = $l_planet_p_under;
		}
		elseif (($porganics + $pore + $pgoods + $penergy + $pfighters + $ptorp + $presearch + $pbuild) > 100.0)
		{
			$result = $l_planet_p_over;
		}
		else
		{
			$debug_query = $db->Execute("UPDATE $dbtables[planets] SET prod_ore=$pore,prod_organics=$porganics,prod_goods=$pgoods,prod_energy=$penergy,prod_fighters=$pfighters,prod_torp=$ptorp,prod_research=$presearch,prod_build=$pbuild WHERE planet_id=$planetinfo[planet_id]");
			db_op_result($debug_query,__LINE__,__FILE__);
