...
if (!empty($traderoute_id))
{
	...
	{
		$smarty->assign("error_msg", $l_tdr_editerr);
	...
	{
		$smarty->assign("error_msg", $l_tdr_notowner);
		$smarty->assign("gotomain", $l_global_mmenu);
		$smarty->display($templatename."traderoute_die.tpl");
		include ("footer.php");
		die();
...
$smarty->assign("l_tdr_editinga", $l_tdr_editinga);
...
$smarty->assign("source_type", $editroute['source_type']);
$smarty->assign("editsource_id", $editroute['source_id']);
...
if ($num_planets != 0)
{
	...
	while ($planetcount < $num_planets)
	{
		...
		if ($planets[$planetcount]['planet_id'] == $editroute['dest_id']){
...
$smarty->assign("planetdestselected", $planetdestselected);
...
$smarty->assign("planetdestselectedteam", $planetdestselectedteam);
...
$smarty->assign("dest_type", $editroute['dest_type']);
$smarty->assign("l_tdr_selendpoint", $l_tdr_selendpoint);
$smarty->assign("l_tdr_planet", $l_tdr_planet);
$smarty->assign("planetsectoridteam", $planetsectoridteam);
$smarty->assign("editdest_id", $editroute['dest_id']);
...
$smarty->assign("l_tdr_modify", $l_tdr_modify);
$smarty->assign("l_tdr_returnmenu", $l_tdr_returnmenu);
$smarty->assign("move_type", $editroute['move_type']);
$smarty->assign("circuit", $editroute['circuit']);
$smarty->assign("editing", $editroute['traderoute_id']);

$smarty->assign("gotomain", $l_global_mmenu);
$smarty->display($templatename."traderoute_edit.tpl");
include ("footer.php");
