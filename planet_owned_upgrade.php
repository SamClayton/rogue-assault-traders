...
if ($planetinfo['owner'] == $playerinfo['player_id'] || ($planetinfo['team'] == $playerinfo['team'] && $playerinfo['team'] > 0 && $planetinfo[owner] > 0))
{
		...
		else
		{
			...
			$smarty->assign("planet_cloak", dropdown("cloak_upgrade",$planetinfo['cloak'], 54));
			$smarty->assign("planetjammer", $planetinfo['jammer']);
			$smarty->assign("planet_jammer", dropdown("jammer_upgrade",$planetinfo['jammer'], 54));
			$smarty->assign("planetshields", $planetinfo['shields']);
			$smarty->assign("planet_shields", dropdown("shields_upgrade",$planetinfo['shields'], 54));
			$smarty->assign("planet_overall", dropdown("overall_upgrade",0, 54));
			$smarty->assign("planettorps", $planetinfo['torp_launchers']);
			$smarty->assign("planet_torps", dropdown("torp_launchers_upgrade",$planetinfo['torp_launchers'], 54));
			$smarty->assign("planetbeams", $planetinfo['beams']);
			$smarty->assign("planet_beams", dropdown("beams_upgrade",$planetinfo['beams'], 54));
			$smarty->assign("planetsensors", $planetinfo['sensors']);
			$smarty->assign("planet_sensors", dropdown("sensors_upgrade",$planetinfo['sensors'], 54));
			$smarty->assign("planetcomputer", $planetinfo['computer']);
			$smarty->assign("planet_computer", dropdown("computer_upgrade",$planetinfo['computer'], 54));
			...
			$smarty->assign("l_by_placebounty", $l_by_placebounty);
			$smarty->assign("gotomain", $l_global_mmenu);
			$smarty->display($templatename."planet_owneddefenses.tpl");
			include ("footer.php");
			die();
		}
