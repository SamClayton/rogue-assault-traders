...
// 
// File: planet_owned_teamcash.php
...
if ($planetinfo['owner'] == $playerinfo['player_id'] || ($planetinfo['team'] == $playerinfo['team'] && $playerinfo['team'] > 0 && $planetinfo[owner] > 0))
{
	...
	if ($playerinfo['player_id'] == $planetinfo['owner'])
	{
		...
		{
			$smarty->assign("result", $l_planet_not_teamcash);
			$debug_query = $db->Execute("UPDATE $dbtables[planets] SET team_cash='N' WHERE planet_id=$planet_id and owner=$playerinfo[player_id]");
			db_op_result($debug_query,__LINE__,__FILE__);
		}
		else
		{
			$smarty->assign("result", $l_planet_teamcash);
			$debug_query = $db->Execute ("UPDATE $dbtables[planets] SET team_cash='Y' WHERE planet_id=$planet_id and owner=$playerinfo[player_id]");
			db_op_result($debug_query,__LINE__,__FILE__);
		}
	...
	{
		$smarty->assign("result", $l_command_no);
		$smarty->assign("planet_id", $planet_id);
		$smarty->assign("l_clickme", $l_clickme);
		$smarty->assign("l_toplanetmenu", $l_toplanetmenu);
		$smarty->assign("l_igb_term", $l_igb_term);
