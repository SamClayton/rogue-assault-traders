...
function sql_insert_identity_on($table)
{
    ...
    $debug_query = $db->Execute("SET IDENTITY_INSERT $table ON;");       
...
function sql_insert_identity_off($table)
{
    ...
    $debug_query = $db->Execute("SET IDENTITY_INSERT $table OFF;");      
    db_op_result($debug_query,__LINE__,__FILE__);
    return $debug_query;
}
...
{
    global $db, $dbtables;
    $debug_query = $db->Execute("SELECT * from $dbtables[players] WHERE (DATEDIFF(second, '1970-01-01 00:00:00', CURRENT_TIMESTAMP) - DATEDIFF(second, '1970-01-01 00:00:00', $dbtables[players].last_login)) / 60 < 5 and email NOT LIKE '%@npc'");
    db_op_result($debug_query,__LINE__,__FILE__);
    return $debug_query;
}
...
function isLoanPending($player_id)
{
    ...
    $debug_query = $db->Execute("SELECT loan, loantime from $dbtables[ibank_accounts] WHERE player_id=$player_id and (loan != 0) and (((DATEDIFF(second, '1970-01-01 00:00:00', CURRENT_TIMESTAMP) - DATEDIFF(second, '1970-01-01 00:00:00', loantime)) / 60) > $igb_lrate) ");
    db_op_result($debug_query,__LINE__,__FILE__);
    if (!$debug_query->EOF)
    {
        return true;
...
if($showzeroranking == 1)
    ...
    $debug_query = $db->Execute("SELECT $dbtables[players].experience, $dbtables[players].email, $dbtables[players].score, " .
                        ...
                        "DATEDIFF(second, '1970-01-01 00:00:00', $dbtables[players].last_login) as last_login, " .
                        "DATEDIFF(second, '1970-01-01 00:00:00', $dbtables[players].last_login) as online, " .
...
{
    global $db, $dbtables;
    $debug_query = $db->SelectLimit("SELECT DATEDIFF(second, '1970-01-01 00:00:00', last_run) as last_run FROM $dbtables[scheduler]",1);
    db_op_result($debug_query,__LINE__,__FILE__);
    return $debug_query;
}
