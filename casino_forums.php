...
include("languages/$langdir/lang_casinoforums.inc");
...
$debug_query = $db->Execute("select * from $dbtables[casino_forums] where casino_sector=$sectorinfo[sector_id]");
...
if($debug_query->RecordCount() == 1)
{
	if($command == "showtopics"){
		...
		$debug_query = $db->Execute("select * from $dbtables[casino_topics] where forum_id=$forumdata[forum_id] order by topic_status desc,lastpostdate desc");
		db_op_result($debug_query,__LINE__,__FILE__);
		...
		if ($debug_query && $reccount > 0){
			...
			while (!$debug_query->EOF){
				...
				$query2 = $db->Execute("select * from $dbtables[casino_posts] where topic_id=$topicinfo[topic_id] order by post_time");
				...
				$query2 = $db->Execute("select * from $dbtables[casino_posts] where topic_id=$topicinfo[topic_id] and post_time>='$playerinfo[forum_login]' order by post_time");
				db_op_result($query2,__LINE__,__FILE__);
				...
				$query2 = $db->Execute("select * from $dbtables[casino_posts] where topic_id=$topicinfo[topic_id] and post_player_id='$playerinfo[player_id]'");
				db_op_result($query2,__LINE__,__FILE__);
...
*/
		...
		$smarty->display($templatename."casinoforum-showtopics.tpl");
		include ("footer.php");
		die();
	}
	...
	if($command == "posttopic"){
		...
		$smarty->assign("isadmin", ($playerinfo['player_id'] == 1));
		$smarty->assign("l_team_notmember", $l_team_notmember);
		$smarty->assign("l_forums_showtopic", $l_forums_showtopic);
		$smarty->assign("gotomain", $l_global_mmenu);
		$smarty->display($templatename."casinoforum-posttopic.tpl");
		include ("footer.php");
		die();
	}
	...
	if($command == "finishtopic"){
		...
		$query=$db->Execute("select * from $dbtables[casino_forums] where forum_id=$forumdata[forum_id] and private=1");
		db_op_result($query,__LINE__,__FILE__);
		$private=$query->RecordCount();

		$query=$db->Execute("insert into $dbtables[casino_topics] (topic_title, topic_poster, topic_time, topic_views, topic_replies, forum_id, topic_status, lastpostdate, post_player_id) values ('". clean_words($topictitle) . "', '$playerinfo[character_name]', '$time', 1, 0, $forumdata[forum_id], $sticky, '$time', '$playerinfo[player_id]')");
		db_op_result($query,__LINE__,__FILE__);

		$debug_query = $db->Execute("select * from $dbtables[casino_topics] where topic_title='". clean_words($topictitle) . "' and topic_poster='$playerinfo[character_name]' and topic_time='$time' and topic_views=1 and topic_replies=0 and forum_id=$forumdata[forum_id] and topic_status=$sticky and lastpostdate='$time'");
		db_op_result($debug_query,__LINE__,__FILE__);
		$topic_id = $debug_query->fields['topic_id'];

		$query=$db->Execute("insert into $dbtables[casino_posts] (topic_id, forum_id, post_time, post_edit_time, post_edit_count, post_username, post_player_id) values ($topic_id, $forumdata[forum_id], '$time', '$time', 0, '$playerinfo[character_name]', $playerinfo[player_id])");
		db_op_result($query,__LINE__,__FILE__);

		$debug_query = $db->Execute("select * from $dbtables[casino_posts] where topic_id='$topic_id' and forum_id=$forumdata[forum_id] and post_time='$time' and post_edit_time='$time' and post_edit_count=0 and post_username='$playerinfo[character_name]' and post_player_id=$playerinfo[player_id]");
		db_op_result($debug_query,__LINE__,__FILE__);
		$post_id = $debug_query->fields['post_id'];

		$query=$db->Execute("insert into $dbtables[casino_posts_text] (post_id, post_text, topic_id, forum_id, post_player_id) values ($post_id, '". clean_words($topicmessage) . "', $topic_id, $forumdata[forum_id], $playerinfo[player_id])");
		db_op_result($query,__LINE__,__FILE__);

		$query=$db->Execute("select * from $dbtables[casino_topics] where forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		$topics=$query->RecordCount();

		$query=$db->Execute("select * from $dbtables[casino_posts] where forum_id=$forumdata[forum_id] and topic_id=$topic_id");
		db_op_result($query,__LINE__,__FILE__);
		$topicposts=$query->RecordCount();

		$query=$db->Execute("update $dbtables[casino_topics] set topic_replies=$topicposts, lastpostdate='$time' where topic_id=$topic_id");
		db_op_result($query,__LINE__,__FILE__);

		$query=$db->Execute("select * from $dbtables[casino_posts] where forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		$posts=$query->RecordCount();

		$query=$db->Execute("update $dbtables[casino_forums] set forum_topics=$topics, forum_posts=$posts, lastposttime='$time' where forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		...
		close_database();
		echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=casino_forums.php?command=readtopic&topic_id=$topic_id\">";
	}
	...
	if($command == "readtopic"){
		...
		$query=$db->Execute("select * from $dbtables[casino_topics] where topic_id=$topic_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		...
		$query=$db->Execute("update $dbtables[casino_topics] set topic_views=$topic_views where topic_id=$topic_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);

		$query=$db->Execute("select * from $dbtables[casino_posts] where topic_id=$topic_id and forum_id=$forumdata[forum_id] order by post_time");
		db_op_result($query,__LINE__,__FILE__);
		$num=$query->RecordCount();
		$smarty->assign("topic_id", $topic_id);
		$smarty->assign("topic_status", $topic_status);
		$smarty->assign("forumadmin", ($playerinfo['player_id'] == 1));
...
*/
				...
				$query2=$db->Execute("select * from $dbtables[casino_posts_text] where post_id=$postinfo[post_id]");
				db_op_result($query2,__LINE__,__FILE__);
				$num2=$query2->RecordCount();
...
*/
		...
		$smarty->assign("l_forums_showtopic", $l_forums_showtopic);
		$smarty->assign("gotomain", $l_global_mmenu);
		$smarty->display($templatename."casinoforum-readtopic.tpl");
		include ("footer.php");
		die();
	}
	...
	if($command == "postreply"){
		...
		$smarty->assign("l_forums_showtopic", $l_forums_showtopic);
		$smarty->assign("gotomain", $l_global_mmenu);
		$smarty->display($templatename."casinoforum-postreply.tpl");
		include ("footer.php");
		die();
	}
	...
	if($command == "finishreply"){
		...
		$query=$db->Execute("update $dbtables[casino_forums] set forum_posts=$posts, lastposttime='$time' where forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		...
		$query=$db->Execute("update $dbtables[casino_topics] set topic_replies=$posts, lastpostdate='$time' where topic_id=$topic_id");
		db_op_result($query,__LINE__,__FILE__);
		...
		unset($_SESSION['currentprogram'], $currentprogram);
		close_database();
		echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=casino_forums.php?command=readtopic&topic_id=$topic_id#$post_id\">";
	}
	...
	if($command == "edit"){
		...
		$query=$db->Execute("select * from $dbtables[casino_posts_text] where post_id=$post_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		...
		$smarty->assign("l_forums_showtopic", $l_forums_showtopic);
		$smarty->assign("gotomain", $l_global_mmenu);
		$smarty->display($templatename."casinoforum-edit.tpl");
		include ("footer.php");
		die();
	}
	...
	if($command == "finishedit"){
		...
		$query=$db->Execute("select * from $dbtables[casino_posts] where post_id=$post_id");
		db_op_result($query,__LINE__,__FILE__);
		$post_edit_count = $query->fields['post_edit_count'];

		$post_edit_count++;

		$query=$db->Execute("update $dbtables[casino_posts] set post_edit_time='$time', post_edit_count=$post_edit_count where post_id=$post_id");
		db_op_result($query,__LINE__,__FILE__);

		$query=$db->Execute("update $dbtables[casino_posts_text] set post_text='". clean_words($topicmessage) ."' where post_id=$post_id");
		db_op_result($query,__LINE__,__FILE__);
	...
	if($command == "delete"){
		...
		$query=$db->Execute("delete from $dbtables[casino_posts] where post_id=$post_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		$query=$db->Execute("delete from $dbtables[casino_posts_text] where post_id=$post_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
		...
		$query=$db->Execute("select * from $dbtables[casino_posts] where topic_id=$topic_id order by post_time");
		db_op_result($query,__LINE__,__FILE__);
		$num=$query->RecordCount();

		if($num == 0){
			$query=$db->Execute("delete from $dbtables[casino_topics] where topic_id=$topic_id and forum_id=$forumdata[forum_id]");
			db_op_result($query,__LINE__,__FILE__);
			...
			unset($_SESSION['currentprogram'], $currentprogram);
			close_database();
			echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=casino_forums.php?command=showtopics\">";
		}else{
			unset($_SESSION['currentprogram'], $currentprogram);
			close_database();
			echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=casino_forums.php?command=readtopic&topic_id=$topic_id\">";
	...
	if($command == "lock"){
		...
		$query=$db->Execute("select * from $dbtables[casino_topics] where forum_id=$forumdata[forum_id] and topic_id=$topic_id");
		db_op_result($query,__LINE__,__FILE__);
		$topics=$query->RecordCount();
		...
		$query=$db->Execute("update $dbtables[casino_topics] set topic_status=$topic_status where topic_id=$topic_id and forum_id=$forumdata[forum_id]");
		db_op_result($query,__LINE__,__FILE__);
	...
	$smarty->assign("l_forums_nocasino", $l_forums_nocasino);
	$smarty->assign("l_forums_showtopic", $l_forums_showtopic);
	$smarty->assign("gotomain", $l_global_mmenu);
	$smarty->display($templatename."casinoforum-die.tpl");
	include ("footer.php");
	die();
}
